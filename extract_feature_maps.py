...
for idx, fileset in enumerate([train_files,test_files]):
    for file in fileset:
        ...
        img_out = get_mask(img_orig,boxes,0.5)
        img_out2 = get_prediction(img_orig,boxes)
        cv2.imwrite(os.path.join('intermediate', 'train', name + '-mask.png'), img_out)
        cv2.imwrite(os.path.join('intermediate', 'train', name + '.png'), img_out2)
        np.savez_compressed(os.path.join('intermediate', 'train', name), heatmaps=out[0][0])
