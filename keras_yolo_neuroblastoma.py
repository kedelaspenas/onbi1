...
def predict(filename):
    img_or = cv2.imread(filename)
    model = KitModel('darknet_neuroblastoma.npy')
    img = cv2.resize(img_or, (416,416))
    img = img/255.0
    output = model.predict(np.expand_dims(img,0))
    print(output.shape)

    boxes = decode_netout(output[0], yolo_parameter()[0], 1)
    im = draw_boxes(img_or, boxes, ['neuroblastoma'])
...
def get_mask(filename):
    ...
    for box in boxes:
        xmin = int(box.xmin*img_or.shape[0])
        ymin = int(box.ymin*img_or.shape[1])
        xmax = int(box.xmax*img_or.shape[0])
        ymax = int(box.ymax*img_or.shape[1])
